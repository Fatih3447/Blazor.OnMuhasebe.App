...
{
    public partial class Sp_BankaDurum : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            var sqlProc = @"CREATE OR ALTER PROCEDURE [dbo].[BankaDurum]
                        ...
                        DECLARE
                        	...
                        	WHERE mbh.BankaHesapId IS NOT NULL AND (mb.MakbuzTuru=1 OR (mb.MakbuzTuru=4 AND mbh.KendiBelgemiz=0)) AND 
                            ...
                            SELECT  @Cikan = COALESCE(SUM(Tutar),0) FROM AppMakbuzHareketler mbh LEFT JOIN
                        		             AppMakbuzlar mb ON mbh.MakbuzId=mb.Id
                        	WHERE mbh.BankaHesapId IS NOT NULL AND (mb.MakbuzTuru=2 OR (mb.MakbuzTuru=4 AND mbh.KendiBelgemiz=1)) AND 
                        	      mb.SubeId = CONVERT(varchar(MAX), @SubeId) AND 
                                  mb.DonemId = CONVERT(varchar(MAX), @DonemId) AND
                        		  mbh.IsDeleted=0
        ...
        {
            migrationBuilder.Sql("DROP PROC BankaDurum");
        }
    }
}
